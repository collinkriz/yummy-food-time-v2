...
Look at EVERY section of the receipt carefully. The address is often near the top with the restaurant name, or in a delivery details section. Search thoroughly before saying "Not visible".`
    ...
    // If address is not visible, use AI to look it up
    if (orderData.address === 'Not visible' && orderData.restaurant) {
      console.log('Address not found, looking up:', orderData.restaurant);
      
      try {
        const addressLookup = await fetch('https://api.anthropic.com/v1/messages', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-api-key': process.env.ANTHROPIC_API_KEY,
          ...
          body: JSON.stringify({
            ...
            messages: [{
              role: 'user',
              content: `What is the street address of ${orderData.restaurant} in Hazel Park, Michigan? If there are multiple locations, give the one in or nearest to Hazel Park, Michigan. Respond with ONLY the street address (e.g. "123 Main St, Hazel Park, MI 48030") and nothing else. If you cannot find it, respond with "Address not found".`
            }]
          })
        });

        const addressData = await addressLookup.json();
        const addressText = addressData.content
          .filter(item => item.type === 'text')
          .map(item => item.text)
          .join('')
          .trim();

        if (addressText && addressText !== 'Address not found') {
          orderData.address = addressText;
          console.log('Found address:', addressText);
        } else {
          console.log('Could not find address for:', orderData.restaurant);
        }
      } catch (error) {
        console.error('Address lookup error:', error);
        // Keep "Not visible" if lookup fails
      }
    }
...
app.post('/bulk-import', express.json({ limit: '50mb' }), async (req, res) => {
  const client = await pool.connect();
  try {
    ...
    console.log(`Bulk importing ${orders.length} orders...`);
    ...
    for (let i = 0; i < orders.length; i++) {
      try {
        ...
        console.error(`Error importing order ${i + 1}:`, error);
      }
    }

    await client.query('COMMIT');
    
    console.log(`Bulk import complete: ${imported} succeeded, ${failed} failed`);
...
app.put('/orders/:id', async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `UPDATE orders 
       SET restaurant = $1, address = $2, delivery_service = $3, subtotal = $4, 
           delivery_fee = $5, service_fee = $6, tax = $7, discount = $8, tip = $9, total = $10
       WHERE id = $11`,
      [
        restaurant,
        address || 'Not provided',
